...
def request_android_permissions():
    """
    Demander les permissions Android nécessaires au démarrage
    À appeler UNE SEULE fois au lancement de l'application
    """
    if platform != 'android':
        print("📱 Pas sur Android - permissions ignorées")
        return True
    
    print("\n" + "="*60)
    print("📱 DEMANDE DE PERMISSIONS ANDROID")
    print("="*60)
    
    try:
        from android.permissions import request_permissions, Permission, check_permission
        
        # Liste de TOUTES les permissions nécessaires pour ZAHEL
        permissions_list = [
            Permission.INTERNET,
        ...
        # Vérifier d'abord quelles permissions sont déjà accordées
        permissions_to_request = []
        for permission in permissions_list:
            if check_permission(permission):
                print(f"✅ Permission déjà accordée: {permission}")
            else:
                print(f"⏳ Permission à demander: {permission}")
        ...
        print(f"\n📱 Demande de {len(permissions_to_request)} permission(s)...")
        
        # Variable pour suivre le résultat
        permission_result = [False]  # Utiliser une liste pour pouvoir modifier depuis callback
        
        def permission_callback(permissions, results):
            """Callback appelé après la réponse de l'utilisateur"""
            print("\n" + "="*60)
            print("📱 RÉSULTAT DES PERMISSIONS")
            print("="*60)
            
            all_granted = True
            for i, (permission, result) in enumerate(zip(permissions, results)):
                if result:
                    ...
                    if permission in [Permission.ACCESS_FINE_LOCATION, Permission.ACCESS_COARSE_LOCATION]:
                        print("   ⚠️ ATTENTION: La localisation est essentielle pour ZAHEL!")
                    elif permission == Permission.INTERNET:
                        print("   ⚠️ ATTENTION: Internet est essentiel pour ZAHEL!")
            
            permission_result[0] = all_granted
            
            if all_granted:
                print("\n✅ Toutes les permissions sont accordées - L'application peut fonctionner normalement")
            else:
                print("\n⚠️ Certaines permissions ont été refusées - Certaines fonctionnalités seront limitées")
        ...
        # Attendre un peu que l'utilisateur réponde (ne bloque pas l'UI)
        # Note: request_permissions est asynchrone, on ne peut pas attendre ici
        print("📱 Dialogue de permissions ouvert - Veuillez répondre")
        
        # Planifier une vérification après 3 secondes pour voir le résultat
        def check_result(dt):
            if permission_result[0]:
                print("✅ Permissions vérifiées: OK")
            else:
                print("⚠️ Permissions vérifiées: Certaines permissions manquantes")
        
        Clock.schedule_once(check_result, 3)
        
        return permission_result[0]
        
    except ImportError:
        print("❌ Module android.permissions non disponible")
        print("   Ce n'est pas un problème si vous n'êtes pas sur Android")
        return False
...
def check_location_permission():
    """
    Vérifier spécifiquement la permission de localisation
    À appeler avant d'utiliser la carte/GPS
    """
    if platform != 'android':
        return True
    ...
    try:
        ...
        fine_location = check_permission(Permission.ACCESS_FINE_LOCATION)
        coarse_location = check_permission(Permission.ACCESS_COARSE_LOCATION)
        
        if fine_location or coarse_location:
        ...
        print(f"❌ Erreur vérification localisation: {e}")
        return False

def show_permission_warning(permission_name):
    """
    Afficher un popup d'avertissement quand une permission est refusée
    """
    from kivy.uix.popup import Popup
    from kivy.uix.label import Label
    from kivy.uix.boxlayout import BoxLayout
    ...
    messages = {
        'location': "L'application a besoin de votre position pour fonctionner correctement.\n\nVeuillez activer la localisation dans les paramètres.",
        'internet': "L'application a besoin d'Internet pour communiquer avec le serveur.",
        'storage': "L'application a besoin d'accéder au stockage pour sauvegarder le cache des cartes."
    }
    
    message = messages.get(permission_name, f"Permission {permission_name} requise")
    
    lbl_title = Label(text="[b][color=FF0000]⚠️ PERMISSION REQUISE[/color][/b]", 
                      markup=True, size_hint_y=None, height=40)
    lbl_message = Label(text=message, halign='center', size_hint_y=None, height=100)
    
    btn_ok = Button(text="J'AI COMPRIS", size_hint_y=None, height=50,
    ...
    popup = Popup(title='Permission requise',
                  content=content,
                  size_hint=(0.8, 0.5),
                  auto_dismiss=False)
...
def log_debug(category, message):
    """Afficher un log seulement si la catégorie est activée"""
    if Config.should_log(category):
        print(message)

def log_info(category, message):
    """Afficher un log info (toujours visible mais moins fréquent)"""
    # Toujours afficher les infos importantes
    print(message)
...
class ClientLoginScreen(Screen):
    ...
    def __init__(self, **kwargs):
        ...
        # Bouton connexion
        btn_login = Button(
            text=f"[size=18][b]{_('login').upper()}[/b][/size]",
            markup=True,
            size_hint=(1, None),
            height=60,
...
class ClientRegisterScreen(Screen):
    ...
    def __init__(self, **kwargs):
        ...
        # Bouton inscription
        btn_register = Button(
            text=f"[size=18][b]{_('register').upper()}[/b][/size]",
            markup=True,
            size_hint=(1, None),
            height=60,
            background_color=(0.2, 0.6, 0.2, 1),
...
class DashboardScreen(Screen):
        ...
        if hasattr(app, "immatricule"):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if data.get("success"):
                        ...
                        try:
                            ...
                            if self.status_indicator.color == [1, 0, 0, 1]:  # Indisponible
                                # Appeler l'API pour voir si on peut être libéré
                                result = api_client.verifier_statut_conducteur()
                                if result.get('success') and result.get('en_course'):
                                    reste = result.get('minutes_restantes', 0)
                                    if reste > 0:
                                        self.courses_label.text = f"⏳ Course en cours - Libération dans {reste} min"
                                        self.courses_label.color = (1, 0.6, 0, 1)
                        except:
                            pass
                    ...
                    app.show_error_popup(
                        ...
                        f"Le serveur a répondu avec l'erreur {response.status_code}",
                        f"URL: {API_BASE_URL}/api/conducteur/statistiques"
                    )
                ...
                app.show_error_popup(
                    ...
                    "Vérifiez que l'API est lancée sur localhost:5001.",
    ...
    def check_notifications(self, dt=None):
        ...
        # ✅ VÉRIFICATION : Ne vérifier que si le dashboard est l'écran actuel
        if not hasattr(self, 'manager') or self.manager.current != 'dashboard':
            return
    ...
    def play_notification_sound(self):
        ...
        # ✅ AJOUTER LA VIBRATION POUR ANDROID
        if platform == 'android':
            try:
                from android.permissions import check_permission, Permission
                if check_permission(Permission.VIBRATE):
                    ...
                    if vibrator and hasattr(vibrator, 'vibrate'):
                        ...
                        print("⚠️ Vibration non disponible")
                else:
                    print("⚠️ Permission VIBRATE non accordée")
            except Exception as e:
                print(f"⚠️ Erreur vibration: {e}")
    ...
    def verifier_statut_avant_chargement(self, dt=None):
        """Vérifier le statut avant de charger le dashboard"""
        global api_client
    
        print("\n" + "="*60)
        print("🔍 [KIVY DEBUG] Vérification statut conducteur")
        ...
        if not api_client.token:
            print(f"   ❌ Token manquant - api_client.token = {api_client.token}")
            print(f"   Tentative de récupération depuis l'app...")
        
            from kivy.app import App
            app = App.get_running_app()
            if hasattr(app, 'api_client') and app.api_client and app.api_client.token:
                # Recopier le token
                api_client.token = app.api_client.token
                api_client.user_type = app.api_client.user_type
                print(f"   ✅ Token récupéré: {api_client.token[:10]}...")
        ...
        try:
            ...
            print(f"      - peut_accepter: {result.get('peut_accepter')}")
            print(f"      - en_course: {result.get('en_course')}")
            print(f"      - message: {result.get('message')}")
            print(f"      - interruptions_jour: {result.get('interruptions_jour')}")
        
            if result.get('success'):
                if 'debug' in result:
                    print(f"      - debug: {result['debug']}")
            
                if not result.get('en_course') and result.get('message') != 'OK':
                    # Afficher un message si libéré
                    if 'interrompue' in result.get('message', ''):
                        self.show_info_message(result['message'])
                        print(f"   ✅ Message affiché: {result['message']}")
            
                # Mettre à jour l'affichage si besoin
                if not result.get('peut_accepter'):
                    self.courses_label.text = f"⏸️ {result.get('message', 'Indisponible')}"
                    self.courses_label.color = (1, 0.5, 0, 1)
                    print(f"   ⚠️ Statut: {result.get('message')}")
...
class CoursesScreen(Screen):
    ...
    def add_course_card(self, course):
        ...
        trajet_box.add_widget(Label(
            text=f"🟢 [b]{_('departure')}:[/b] {depart_adr}",
            markup=True,
            halign='left',
            font_size='12sp',
            size_hint=(1, 0.5),
        ...
        trajet_box.add_widget(Label(
            text=f"🔴 [b]{_('destination')}:[/b] {arrivee_adr}",
            markup=True,
            halign='left',
            font_size='12sp',
            size_hint=(1, 0.5),
        ...
        if platform == 'android':
            try:
                ...
                if not check_permission(Permission.WRITE_EXTERNAL_STORAGE):
                    print("⚠️ Permission d'écriture non accordée - Cache OSM désactivé")
                    # Utiliser un cache temporaire en mémoire
                    cache_dir = None
                else:
...
class SelectionScreen(Screen):
    def __init__(self, **kwargs):
        ...
        # Bouton Conducteur
        btn_driver = Button(
            text=f"[size=20][b]{_('driver_mode')}[/b][/size]",
            markup=True,
            background_color=(0.2, 0.5, 0.8, 1),
            size_hint=(0.8, None),
            height=80,
        ...
        # Bouton Client
        btn_client = Button(
            text=f"[size=20][b]{_('client_mode')}[/b][/size]",
            markup=True,
        ...
        btn_language.bind(on_press=self.go_to_language)

        # Version
        version = Label(
            text=_('version') + " • © ZAHEL 2025",
            font_size="12sp",
            color=(0.5, 0.5, 0.5, 1),
            size_hint=(1, 0.1),
        )
        ...
        layout.add_widget(btn_language)
...
class MapSelectionScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.lbl_instructions = Label(
            text=f'[size=16][color=2E7D32]📌 {_("select_on_map")}[/color][/size]',
            markup=True,
            halign='center'
        )
        ...
        btn_my_location = Button(
            text=f'[size=16]📍 {_("use_my_location")}[/size]',
            markup=True,
            size_hint=(0.25, 1),
            background_color=(0.2, 0.6, 0.2, 1)
        )
        ...
        self.step_depart = Label(
            text=f'[size=14][b]1. {_("departure")}[/b][/size]',
            markup=True,
            color=(0.2, 0.5, 0.8, 1)
        )
        
        self.step_arrivee = Label(
            text=f'[size=14]2. {_("destination")}[/size]',
            markup=True,
            color=(0.5, 0.5, 0.5, 1)
        )
        
        self.step_confirm = Label(
            text=f'[size=14]3. {_("confirm")}[/size]',
            markup=True,
            color=(0.5, 0.5, 0.5, 1)
        )
...
class OrderRideScreen(Screen):
    def __init__(self, depart_coords=None, arrivee_coords=None, destination="", **kwargs):
        ...
        lbl_dest_title = Label(
            text=f"[size=16][b]{_('departure')}[/b][/size]",
            markup=True,
            halign="left",
            size_hint=(1, 0.4),
        )
        ...
        service_box = BoxLayout(
            orientation="vertical", size_hint=(1, None), height=350, spacing=15
        )

        lbl_service = Label(
            text=f"[size=16][b]{_('services')}[/b][/size]",
            markup=True,
            halign="left",
            size_hint=(1, 0.15),
        )

        service_scroll = ScrollView(size_hint=(1, 0.85), do_scroll_y=True)
        services_container = BoxLayout(
            orientation="vertical", 
            spacing=15, 
            size_hint_y=None
        )
        services_container.bind(minimum_height=services_container.setter('height'))

        # ⭐ GARDE UNE RÉFÉRENCE AU CONTENEUR
        self.services_container = services_container

        # ⭐ CALCULER LA DISTANCE RÉELLE UNE FOIS
        real_distance = self.calculate_real_distance()

        print(f"📏 Distance réelle pour calcul des prix: {real_distance} km")

        # Prix avec la distance réelle
        self.services = [
            {
                "id": "standard",
                "name": _('standard'),
                "icon": "🚗",
                "price": f"{self.calculate_service_price('standard', real_distance):,} KMF",
                "desc": _('standard_desc'),
                "color": (0.2, 0.6, 0.2, 1),
                "real_price": self.calculate_service_price('standard', real_distance)
            },
            {
                "id": "confort",
                "name": _('confort'),
                "icon": "✨",
                "price": f"{self.calculate_service_price('confort', real_distance):,} KMF",
                "desc": _('confort_desc'),
                "color": (0.8, 0.5, 0.2, 1),
                "real_price": self.calculate_service_price('confort', real_distance)
            },
            {
                "id": "moto",
                "name": _('moto'),
                "icon": "🏍️",
                "price": f"{self.calculate_service_price('moto', real_distance):,} KMF",
                "desc": _('moto_desc'),
                "color": (0.2, 0.5, 0.8, 1),
                "real_price": self.calculate_service_price('moto', real_distance)
            },
            {
                "id": "luxe",
                "name": _('luxury'),
                "icon": "⭐",
                "price": f"{self.calculate_service_price('luxe', real_distance):,} KMF",
                "desc": _('luxury_desc'),
                "color": (0.7, 0.2, 0.7, 1),
                "real_price": self.calculate_service_price('luxe', real_distance)
            },
        ]
        ...
        for service in self.services:
            ...
            btn.service_data = service
            btn.bind(on_press=self.select_and_search)
            services_container.add_widget(btn)

        service_scroll.add_widget(services_container)
        service_box.add_widget(lbl_service)
        service_box.add_widget(service_scroll)
        ...
        lbl_price = Label(
            text=f"[size=16][b]{_('estimated_price')}[/b][/size]",
            markup=True,
            halign="left",
            size_hint=(1, 0.3),
        )
        ...
        lbl_price_info = Label(
            text="[size=12]" + _('estimated_price') + " • " + _('no_negotiation') + "[/size]",
            markup=True,
            size_hint=(1, 0.2),
            color=(0.5, 0.5, 0.5, 1),
        )
        ...
        main_layout.add_widget(price_box)

        main_scroll.add_widget(main_layout)
        self.add_widget(main_scroll)

    def calculate_real_distance(self):
        """Calcule la distance réelle entre départ et arrivée"""
        ...
        print(f"📍 calculate_real_distance - Départ: {self.depart_coords}")
        print(f"📍 calculate_real_distance - Arrivée: {self.arrivee_coords}")
    
        # S'assurer que les coordonnées existent
        if not hasattr(self, 'depart_coords') or not self.depart_coords:
            self.depart_coords = (-11.6980, 43.2560)
        if not hasattr(self, 'arrivee_coords') or not self.arrivee_coords:
            self.arrivee_coords = (-11.7100, 43.2650)
        ...
        try:
            ...
            if distance < 0.1:
                distance = 0.5
            print(f"📏 Distance calculée: {round(distance, 2)} km")
    ...
    def on_pre_enter(self):
        """Appelé juste avant d'afficher l'écran - RECALCUL FORCÉ"""
        print("\n" + "="*60)
        print("🔄 OrderRideScreen - RÉACTUALISATION FORCÉE")
        print("="*60)
    
        # 1. Afficher les coordonnées actuelles
        print(f"📍 Départ: {self.depart_coords}")
        print(f"📍 Arrivée: {self.arrivee_coords}")
    
        # 2. Recalculer la distance
        real_distance = self.calculate_real_distance()
        print(f"📏 Nouvelle distance: {real_distance} km")
    
        # 3. Recalculer tous les prix
        for service in self.services:
            old_price = service['real_price']
            service['real_price'] = self.calculate_service_price(service['id'], real_distance)
            service['price'] = f"{service['real_price']:,} KMF"
            print(f"   • {service['name']}: {old_price} → {service['real_price']} KMF")
    
        # 4. Mettre à jour le prix estimé
        self.price = self.calculate_price_from_distance()
        self.lbl_calculated_price.text = f"[size=28][b]{self.price:,} KMF[/b][/size]"
    
        # 5. FORCER LA RECRÉATION DES BOUTONS
        self.recreate_service_buttons()
    
        print(f"✅ Prix mis à jour - Standard: {self.services[0]['real_price']} KMF")
        print("="*60 + "\n")

    def update_prices_with_real_distance(self):
        """Met à jour les prix des services avec la distance réelle"""
        real_distance = self.calculate_real_distance()
        print(f"🔄 Mise à jour des prix avec distance: {real_distance} km")
        ...
        # ⭐ RECRÉER LES BOUTONS AVEC LES NOUVEAUX PRIX
        self.recreate_service_buttons()

    def recreate_service_buttons(self):
        """Recrée les boutons de service avec les prix mis à jour"""
        # Vider le conteneur
        self.services_container.clear_widgets()
    
        # Recréer chaque bouton
        for service in self.services:
            btn = Button(
                text=f"{service['icon']}  [size=18][b]{service['name']}[/b][/size]\n[size=20][b]{service['price']}[/b][/size]\n[size=12]{service['desc']}[/size]",
                markup=True,
            ...
            btn.service_data = service
            btn.bind(on_press=self.select_and_search)
            self.services_container.add_widget(btn)
    ...
    def calculate_price_from_distance(self):
        ...
        # ✅ Récupérer la catégorie sélectionnée avec valeur par défaut
        categorie = getattr(self, 'selected_service', 'standard')
        if categorie not in tarifs_base:
            print(f"⚠️ Catégorie inconnue: {categorie}, utilisation 'standard'")
            categorie = 'standard'
        ...
        if not hasattr(self, 'depart_coords') or not self.depart_coords:
            print("⚠️ depart_coords non défini, utilisation valeurs par défaut")
            self.depart_coords = (-11.6980, 43.2560)
    
        if not hasattr(self, 'arrivee_coords') or not self.arrivee_coords:
            print("⚠️ arrivee_coords non défini, utilisation valeurs par défaut")
            self.arrivee_coords = (-11.7100, 43.2650)
...
class WaitingForDriverScreen(Screen):
    def __init__(self, **kwargs):
        ...
        stats_box.add_widget(Label(
            text=f"[size=16][b]📊 {_('drivers_online')}[/b][/size]",
            markup=True,
            halign="left",
            size_hint=(1, 0.3)
        ))
        ...
        self.lbl_online = Label(
            text=f"• 0 {_('drivers_online')}",
            halign="left",
            size_hint=(1, 0.33)
        )
        
        self.lbl_nearby = Label(
            text=f"• 0 {_('drivers_around')}",
            halign="left",
            size_hint=(1, 0.33)
        )
        
        self.lbl_category = Label(
            text=f"• 0 {_('vehicles_available')} {service.upper()}",
            halign="left",
            size_hint=(1, 0.33)
        )
        ...
        self.lbl_service_price = Label(
            text=f"[b]{self.get_service_name(service)} • {price:,} KMF[/b]",
            markup=True,
            halign="left",
            size_hint=(0.85, 1)
        )
        ...
        # ========== BOUTON ANNULER ==========
        btn_cancel = Button(
            text=f"[size=18][b]{_('cancel_search').upper()}[/b][/size]",
            markup=True,
            size_hint=(0.8, 0.08),
            background_color=(0.8, 0.2, 0.2, 1),
            color=(1, 1, 1, 1),
    ...
    def get_service_name(self, service):
        ...
        return names.get(service, service).capitalize()
    ...
    def go_to_tracking(self, driver_info):
        ...
        normalized_driver = {
            ...
            'vehicule': driver_info.get('vehicule', driver_info.get('vehicle', 'Véhicule')),
            'plaque': driver_info.get('plaque', driver_info.get('plate', 'XX-000-XX')),
            'note': driver_info.get('note', driver_info.get('rating', 5.0)),
            'courses': driver_info.get('courses', driver_info.get('trips', 0)),
            'couleur': driver_info.get('couleur', driver_info.get('color', '')),
            'prix': driver_info.get('price', self.price),
...
class DriverTrackingScreen(Screen):
    def __init__(self, driver=None, destination="", price=0, **kwargs):
        ...
        # Normalisation des données conducteur
        if driver:
            self.driver = {
                'nom': driver.get('nom') or driver.get('name', _('driver')),
                'vehicule': driver.get('vehicule') or driver.get('vehicle', _('vehicle')),
                'plaque': driver.get('plaque') or driver.get('plate', 'XX-000-XX'),
                'note': driver.get('note') or driver.get('rating', 5.0),
                'courses': driver.get('courses') or driver.get('trips', 0),
                'couleur': driver.get('couleur') or driver.get('color', _('white')),
                'prix': driver.get('prix') or driver.get('price', price),
                'telephone': driver.get('telephone') or driver.get('phone', ''),
                'eta': driver.get('eta', '~4 ' + _('min'))
            }
        else:
            self.driver = {
                "nom": _('driver'),
        ...
        try:
            ...
            self.driver_marker = MapMarker(lat=-11.6980, lon=43.2560)
            self.client_marker = MapMarker(lat=-11.6980, lon=43.2560)
            self.dest_marker = MapMarker(lat=-11.7100, lon=43.2650)
        ...
        self.lbl_vehicle = Label(
            text=f'[size=18]🚗 {self.driver["couleur"]} {self.driver["vehicule"]}[/size]',
            markup=True,
            halign="left",
            size_hint=(0.6, 1)
        )
        ...
        self.lbl_plate = Label(
            text=f'[size=16][b]{self.driver["plaque"]}[/b][/size]',
            markup=True,
            halign="right",
            size_hint=(0.4, 1)
        )
        ...
        driver_section.add_widget(vehicle_row)

        # ----- Ligne 2 : Nom et note -----
        info_row = BoxLayout(size_hint=(1, 0.2))
    
        self.lbl_name = Label(
            text=f'[size=18][b]{self.driver["nom"]}[/b][/size]\n★ {self.driver["note"]} ({self.driver["courses"]} {_("rides")})',
            markup=True,
            halign="left",
            size_hint=(0.7, 1)
        )
        ...
        self.lbl_eta = Label(
            text=f"[size=22][b]{self.driver['eta']}[/b][/size]",
            markup=True,
            halign="right",
            size_hint=(0.3, 1)
        )
        ...
        driver_section.add_widget(info_row)

        # ----- Ligne 3 : CHRONO -----
        chrono_box = BoxLayout(size_hint=(1, 0.15))
        self.chrono_label = Label(
            text=f"⏱️ {_('estimated_arrival')}: --:--",
            font_size="16sp",
            bold=True,
            color=(0.2, 0.5, 0.8, 1),
            halign='center'
        ...
        driver_section.add_widget(chrono_box)
        ...
        driver_section.add_widget(progress_row)
        ...
        lbl_dest = Label(
            text=f"[size=16]{_('destination')}:[/size] [b]{self.destination}[/b]",
            markup=True,
            halign="left",
            size_hint=(1, 0.5),
        )
        ...
        lbl_price = Label(
            text=f"[size=18][b]{_('price')}: {self.price} KMF[/b][/size]",
            markup=True,
            halign="left",
            size_hint=(1, 0.5),
        )
        ...
        # ========== BOUTON ANNULER ==========
        btn_cancel = Button(
            text=f"[size=16]{_('cancel_ride')}[/size]",
            markup=True,
            size_hint=(1, 0.1),
        ...
        print("✅ DriverTrackingScreen initialisé")
    ...
    def update_from_api(self, dt=None):
        """Mettre à jour les informations depuis l'API"""
    
        if not hasattr(self, 'manager') or self.manager.current != 'driver_tracking':
            return
    
        global api_client

        if not hasattr(self, 'course_code') or not self.course_code:
            if hasattr(self, '_last_log_time') and time.time() - self._last_log_time > 5:
                print("ℹ️ En attente d'une course...")
        ...
        try:
            ...
            if result and result.get('success'):
                ...
                if conducteur:
                    ...
                    # ✅ METTRE À JOUR LE DICTIONNAIRE NORMALISÉ
                    self.driver.update({
                        'nom': conducteur.get('nom', self.driver.get('nom', 'Inconnu')),
                        'telephone': conducteur.get('telephone', self.driver.get('telephone', '')),
                        'vehicule': f"{conducteur.get('marque_vehicule', '')} {conducteur.get('modele_vehicule', '')}".strip(),
                        'couleur': conducteur.get('couleur_vehicule', self.driver.get('couleur', '')),
                        'plaque': conducteur.get('immatricule', self.driver.get('plaque', '')),
                        'note': conducteur.get('note_moyenne', self.driver.get('note', 5.0)),
                        'courses': conducteur.get('courses_effectuees', self.driver.get('courses', 0))
                    })
        
                    # ✅ Mettre à jour les labels
                    if hasattr(self, 'lbl_name'):
                        self.lbl_name.text = f'[size=22][b]{self.driver["nom"]}[/b][/size]\n★ {self.driver["note"]} ({self.driver["courses"]} courses)'
        
                    if hasattr(self, 'lbl_vehicle'):
                        self.lbl_vehicle.text = f'[size=20]🚗 {self.driver["couleur"]} {self.driver["vehicule"]}[/size]'
        
                    if hasattr(self, 'lbl_plate'):
                        self.lbl_plate.text = f'[size=20][b]{self.driver["plaque"]}[/b][/size]'
    ...
    def contact_driver(self, instance):
        ...
        print(f"📞 Contacter le conducteur: {self.driver['nom']}")
    
        # Ouvrir le téléphone si numéro disponible
        if self.driver.get('telephone'):
            import webbrowser
            tel_url = f"tel:{self.driver['telephone']}"
            print(f"📱 Ouverture du téléphone: {tel_url}")
            webbrowser.open(tel_url)
        else:
            # Popup d'information
            self.show_info_popup("Contact", "Numéro de téléphone non disponible")
    ...
    def share_ride(self, instance):
        ...
        # ✅ CORRECTION : Utiliser les clés normalisées
        share_text = f"Je suis en route avec ZAHEL. Conducteur: {self.driver['nom']}, Véhicule: {self.driver['couleur']} {self.driver['vehicule']}, ETA: {self.driver.get('eta', '~4 min')}"
        print(f"Texte à partager: {share_text}")
    
        # Copier dans le presse-papier
        from kivy.core.clipboard import Clipboard
        Clipboard.copy(share_text)
        self.show_info_popup("Partage", "Informations copiées dans le presse-papier")

    def show_payment(self, instance):
        """Afficher les détails du paiement"""
        # ✅ CORRECTION : Utiliser 'prix' au lieu de 'price'
        montant = self.driver.get('prix', self.price)
        print(f"Détails du paiement: {montant} KMF")
        ...
        lbl = Label(
            ...
            f"Montant: [b]{montant} KMF[/b]\n"
            f"Mode: 💵 Espèces\n"
            f"Conducteur: {self.driver['nom']}\n"
    ...
    def on_leave(self):
        """Quand on quitte l'écran"""
        print("🚪 Quitte l'écran de suivi")
        self._update_enabled = False  # Désactiver les mises à jour
        # Arrêter le chrono si présent
        if hasattr(self, 'update_timer'):
            Clock.unschedule(self.update_timer)

    def get_driver_info(self, key, default=""):
        """Obtenir une information du conducteur de manière sécurisée"""
        if key in self.driver:
        ...
        if key in key_mapping and key_mapping[key] in self.driver:
            return self.driver[key_mapping[key]]
    
        print(f"⚠️ Clé '{key}' non trouvée dans driver")
        return default

    def show_info_popup(self, title, message):
        """Afficher un popup d'information simple"""
        from kivy.uix.popup import Popup
        from kivy.uix.label import Label
        from kivy.uix.boxlayout import BoxLayout
        from kivy.uix.button import Button
...
class LanguageSelectionScreen(Screen):
    ...
    def __init__(self, **kwargs):
        ...
        btn_fr.bind(on_press=lambda x: self.select_language('fr'))
        ...
        btn_en.bind(on_press=lambda x: self.select_language('en'))
    ...
    def select_language(self, lang):
        """Changer la langue et aller à la sélection"""
        set_language(lang)
    
        # Sauvegarder le choix
        try:
            with open('language_pref.json', 'w') as f:
                json.dump({'lang': lang}, f)
            print(f"🌐 Langue sauvegardée: {lang}")
        except Exception as e:
            print(f"⚠️ Erreur sauvegarde langue: {e}")
...
class ZAHELApp(App):
    ...
    def load_language_preference(self):
        ...
        try:
            ...
            if os.path.exists('language_pref.json'):
                with open('language_pref.json', 'r') as f:
                    data = json.load(f)
                    lang = data.get('lang', 'fr')
                    set_language(lang)
                    print(f"🌐 Langue chargée: {lang}")
            else:
                # Par défaut français
                set_language('fr')
                print("🌐 Langue par défaut: fr")
        except Exception as e:
            print(f"⚠️ Erreur chargement langue: {e}")
            set_language('fr')

    # ⭐ NOUVELLE MÉTHODE 1 : Configurer l'écouteur d'intents
    def setup_intent_listener(self, dt):
        """Écouter les intents entrants (localisations)"""
        try:
            from android import activity
            from jnius import autoclass
            
            PythonActivity = autoclass('org.kivy.android.PythonActivity')
            self.current_intent = PythonActivity.mIntent
            self._handle_intent(self.current_intent)
            
            # Écouter les nouveaux intents
            activity.bind(on_new_intent=self._on_new_intent)
            
            print("✅ [INTENT] Listener configuré avec succès")
        except Exception as e:
            print(f"❌ [INTENT] Erreur configuration: {e}")
    
    # ⭐ NOUVELLE MÉTHODE 2 : Gérer les nouveaux intents
    def _on_new_intent(self, intent):
        """Appelé quand un nouvel intent arrive (ex: clic sur localisation)"""
        print(f"✅ [INTENT] Nouvel intent reçu!")
        self._handle_intent(intent)
    
    # ⭐ NOUVELLE MÉTHODE 3 : Traiter l'intent
    def _handle_intent(self, intent):
        """Traite un intent et extrait les coordonnées"""
        try:
            ...
            # CAS 1 : Intent geo: (format: geo:latitude,longitude)
            if data.startswith('geo:'):
                self._handle_geo_intent(data)
            
            # CAS 2 : Intent Google Maps (http://maps.google.com/?q=lat,lng)
            elif 'maps.google.com' in data or 'maps.app.goo.gl' in data:
                self._handle_maps_intent(data)
                
        except Exception as e:
            print(f"❌ [INTENT] Erreur traitement: {e}")
    
    # ⭐ NOUVELLE MÉTHODE 4 : Gérer les intents geo:
    def _handle_geo_intent(self, data):
        """Extrait les coordonnées d'un intent geo:"""
        try:
            # Format: geo:latitude,longitude
            # ou geo:0,0?q=latitude,longitude
            coords_part = data[4:].split('?')[0]
            ...
            if len(coords) >= 2:
                ...
                print(f"📍 [INTENT] Coordonnées extraites: {lat}, {lng}")
                
                # Stocker dans l'application
                self.received_location = {
                    'lat': lat,
                    'lng': lng,
                    'adresse': f'Localisation reçue ({lat:.6f}, {lng:.6f})'
                }
                
                # Aller à l'écran de commande
                Clock.schedule_once(lambda dt: self.go_to_order_with_location(lat, lng), 0.5)
                
        except Exception as e:
            print(f"❌ [INTENT] Erreur parsing geo: {e}")
    
    # ⭐ NOUVELLE MÉTHODE 5 : Gérer les intents Google Maps
    def _handle_maps_intent(self, data):
        ...
        try:
            ...
            if 'q' in query:
                q_value = query['q'][0]
                if ',' in q_value:
                    coords = q_value.split(',')
                    lat = float(coords[0].strip())
                    lng = float(coords[1].strip())
                    
                    print(f"📍 [INTENT] Coordonnées Google Maps: {lat}, {lng}")
                    
                    self.received_location = {
                        'lat': lat,
                        'lng': lng,
                        'adresse': f'Google Maps ({lat:.6f}, {lng:.6f})'
                    }
                    
                    Clock.schedule_once(lambda dt: self.go_to_order_with_location(lat, lng), 0.5)
                    
        except Exception as e:
            print(f"❌ [INTENT] Erreur parsing maps: {e}")
    
    # ⭐ NOUVELLE MÉTHODE 6 : Aller à l'écran de commande avec localisation
    def go_to_order_with_location(self, lat, lng):
        ...
        if not hasattr(self, 'client_data') or not self.client_data:
            ...
            # Stocker la localisation pour après connexion
            self.pending_location = {'lat': lat, 'lng': lng}
            
            # Aller à l'écran de connexion
            if 'client_login' in self.sm.screen_names:
                self.sm.current = 'client_login'
            else:
                self.sm.current = 'selection'
            return
        
        # Utilisateur connecté - aller directement à la carte
        print("✅ Utilisateur connecté - ouverture carte")
        
        # Stocker dans l'app pour l'écran de carte
        self.selected_destination = {
            'text': f'📍 {lat:.6f}, {lng:.6f}',
            'coordinates': (lat, lng),
            'name': 'Localisation reçue',
            'address_found': True
        }
        
        # Aller à l'écran de sélection sur carte
        if 'map_selection' in self.sm.screen_names:
            self.sm.current = 'map_selection'
        else:
            print("⚠️ Écran map_selection non trouvé")
    ...
    def show_error_popup(self, title, message, details=None):
        """Afficher un popup d'erreur informatif pour l'utilisateur"""
        from kivy.uix.popup import Popup
        from kivy.uix.label import Label
        from kivy.uix.boxlayout import BoxLayout
        from kivy.uix.button import Button
        ...
        # Titre avec erreur
        lbl_title = Label(
            text=f"[b][color=FF0000]{title}[/color][/b]",
            markup=True,
            size_hint_y=None,
            height=30
        )
        ...
        if details:
            lbl_details = Label(
                text=f"[size=12][color=666666]{details}[/color][/size]",
                markup=True,
                halign='center',
                size_hint_y=None,
                height=40
        ...
        # Planifier l'ouverture sur le thread principal
        Clock.schedule_once(lambda dt: popup.open(), 0)

    def handle_api_error(self, error_result, context=""):
        """Gérer les erreurs API de manière uniforme"""
        error = error_result.get('error', 'Erreur inconnue')
        details = error_result.get('details', '')
    
        print(f"❌ API Error [{context}]: {error}")
        if details:
            print(f"   Détails: {details}")
    
        # Messages utilisateur selon le type d'erreur
        user_messages = {
            'Non authentifié': "Votre session a expiré. Veuillez vous reconnecter.",
            'Connexion API impossible': "Impossible de contacter le serveur. Vérifiez votre connexion.",
            'HTTP 404': "Service non trouvé. L'API est-elle à jour ?",
            'HTTP 500': "Erreur serveur. Veuillez réessayer plus tard.",
            'HTTP 400': "Requête invalide. L'application est peut-être obsolète.",
        }
    
        # Chercher un message personnalisé
        user_message = None
        for key, msg in user_messages.items():
            if key in error:
                user_message = msg
                break
    
        if not user_message:
            user_message = f"Une erreur est survenue: {error}"
    
        # Afficher le popup
        self.show_error_popup("Erreur API", user_message, details)
    
        return user_message

    def test_api_connection_at_startup(self):
        """Tester la connexion API au démarrage et informer l'utilisateur"""
        global api_client
        ...
        def check_connection(dt):
            if api_client:
                try:
                    if api_client.test_connection():
                        print("✅ Connexion API OK au démarrage")
                    else:
                        print("⚠️ API inaccessible au démarrage")
                    ...
                    print("⚠️ Erreur test connexion API")
    
        # Vérifier après 2 secondes (le temps que tout se charge)
        Clock.schedule_once(check_connection, 2)

    def show_warning_popup(self, title, message):
        """Afficher un popup d'avertissement (non bloquant)"""
        from kivy.uix.popup import Popup
        from kivy.uix.label import Label
        from kivy.uix.boxlayout import BoxLayout
        from kivy.uix.button import Button
    ...
    def handle_missing_permissions(self):
        """Gérer le cas où les permissions critiques sont refusées"""
        if platform != 'android':
            return
        ...
        try:
            ...
            has_internet = check_permission(Permission.INTERNET)
            has_location = (check_permission(Permission.ACCESS_FINE_LOCATION) or 
                            check_permission(Permission.ACCESS_COARSE_LOCATION))
        
            if not has_internet:
                print("❌ PERMISSION INTERNET REFUSÉE - L'application ne peut pas fonctionner")
                self.show_fatal_error(
                    "Permission Internet refusée",
                    "L'application a besoin d'Internet pour fonctionner.\n"
                    "Veuillez activer la permission dans les paramètres."
                )
        
            if not has_location and (IS_DRIVER_MODE or self.sm.current in ['map_selection', 'navigation']):
                print("⚠️ PERMISSION LOCALISATION REFUSÉE - Fonctionnalités limitées")
                self.show_warning_popup(
                    "Localisation désactivée",
                    "Certaines fonctionnalités de navigation seront limitées.\n"
    ...
    def show_fatal_error(self, title, message):
        """Afficher une erreur fatale (bloquante)"""
        from kivy.uix.popup import Popup
        from kivy.uix.label import Label
        from kivy.uix.boxlayout import BoxLayout
        from kivy.uix.button import Button
        ...
        lbl_title = Label(text=f"[b][color=FF0000]{title}[/color][/b]", 
                          markup=True, size_hint_y=None, height=40)
        lbl_message = Label(text=message, halign='center', size_hint_y=None, height=100)
    
        btn_quit = Button(text="QUITTER", size_hint_y=None, height=50,
                          background_color=(0.8, 0.2, 0.2, 1))
        ...
        popup = Popup(title='❌ ERREUR FATALE',
                      content=content,
                      size_hint=(0.8, 0.5),
                      auto_dismiss=False)
        ...
        btn_quit.bind(on_press=quit_app)
        popup.open()
